...
import { AutoSizer, MultiGrid, GridCellRenderer, Index } from 'react-virtualized'
...
const rowData = (rowIndex: number, columnIndex: number) =>
  columnIndex === 0 ? foodName[rowIndex-1] : foodNutrients[nutrientKey[columnIndex-1]].Varde[rowIndex-1];
...
const cellRenderer: GridCellRenderer
  = ({ rowIndex, columnIndex, key, style }) =>
    <div key={key} style={style}>{cellData(rowIndex, columnIndex)}</div>;
